...
module.exports = async function ({
  ...
  try {
    ...
    for (const log of analyzeLogs) {
      ...
      const parsedIssues = parseAnalyzerOutputs(
        analyzerOutput,
        workingDir,
        filterLevel
      );
      logVerbose(
        `Parsed issues for ${log}: ${JSON.stringify(parsedIssues, null, 2)}`
      );
  ...
  try {
    ...
    for (const log of customLintLogs) {
      ...
      logVerbose(
        `Parsed custom lint issues for ${log}: ${JSON.stringify(
          customLintIssues,
          null,
          2
        )}`
  ...
  try {
    ...
    const maxIssuesComment = response.data.find((comment) =>
      comment.body.includes(maxIssuesCommentHeader)
    );
  ...
  if (issues.length > maxIssues) {
    ...
    try {
      ...
      logVerbose(
        `Number of issues exceeds maximum: ${issues.length} > ${maxIssues}`
      );
      return;
    } catch (error) {
  ...
  try {
    ...
    logVerbose(
      `Issues not in Diff: ${JSON.stringify(issuesNotInDiff, null, 2)}`
    );
    ...
    outlineComment =
      issuesNotInDiff.length > 0
        ? generateTableForIssuesNotInDiff(issuesNotInDiff)
        : null;
  ...
  try {
    ...
    const existOutlineComment = response.data.find((comment) =>
  ...
  try {
    ...
    existingReviewComments = response.data.map(
      (comment) =>
        new RemoteComment(
          comment.id,
          comment.path,
    ...
    logVerbose(
      `Existed remote comments: ${JSON.stringify(
  ...
  const commentsToAdd = inlineComments.filter(
    (local) =>
      !existingReviewComments.some((remote) =>
        remote.matchesLocalComment(local)
      )
  );
  const commentsToDelete = existingReviewComments
    .filter((comment) => comment.hasFlutterAnalyzeCommentHeader())
    .filter(
      (remote) =>
...
class Comment {
  constructor(issues) {
    ...
    this.body += issues
      .map((issue) => {
        return `<tr><td>${levelIcon[issue.level]}</td><td>${
          issue.message
        }</td></tr>`;
      })
      .join("");
...
function parseAnalyzerOutputs(analyzeLog, workingDir, filterLevel) {
  ...
  while ((match = regex.exec(analyzeLog))) {
    // skip if level is in filterLevels
    if (filterLevels.includes(match[1].toLowerCase())) {
      continue;
    }
    issues.push(
      new Issue(
...
function generateTableForIssuesNotInDiff(issuesNotInDiff) {
  ...
  return (
    `<p>Flutter Analyze Commenter has detected the following issues, including those within your commits, and additional potential issues due to recent updates to the base branch:</p>` +
    `<table>` +
